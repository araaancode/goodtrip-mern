      ...
      <div className="max-w-6xl mx-auto">
        ...
        {items.length === 0 ? (
          <div className="bg-white p-12 rounded-xl shadow-sm text-center max-w-2xl mx-auto border border-gray-100">
            ...
            <p className="mt-3 text-gray-600 max-w-md mx-auto">می‌توانید با مراجعه به صفحه سفارش غذا، غذای مورد نظر خود را انتخاب کنید</p>
            <Link
              to="/order-food"
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                <ul className="divide-y divide-gray-100">
                  {items.map((item) => (
                    <li key={item.food._id} className="p-6 hover:bg-gray-50 transition">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                        <div className="flex items-start gap-5">
                          <div className="relative flex-shrink-0">
                            <img
                              className="w-28 h-28 object-cover rounded-lg border border-gray-200"
                              src={item.food.photo || '/images/food-placeholder.jpg'}
                              alt={item.food.name}
                            />
                          ...
                          <div>
                            <h3 className="text-xl font-semibold text-gray-800">{item.food.name}</h3>
                            <p className="text-gray-500 mt-1">{isClient ? item.food.price.toLocaleString('fa-IR') : '0'} تومان</p>
                            <p className="text-sm text-gray-400 mt-2">توسط: {item.food.cookName}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <span className="px-4 py-2 bg-white text-center text-gray-700 font-medium min-w-[3rem]">
                              {isClient ? item.quantity.toLocaleString('fa-IR') : '0'}
                            </span>
                            <button
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-6 border border-gray-100">
                ...
                >
                  {isLoading ? (
                    ...
                    <Link className='inline' to="/create-order">
                     تکمیل سفارش
                    </Link>
                  )}
                </button>
