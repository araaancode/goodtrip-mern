...
const NotificationsPage = () => {
  ...
  const loadNotifications = async () => {
    try {
      setLoading(true);
      // Replace with actual API call to get user's notifications
  ...
  const markAllAsRead = async () => {
    try {
      // Replace with actual API call
      // await axios.patch('/api/users/notifications/mark-all-read');
      setNotifications(notifications.map(notification => ({
        ...notification,
        read: true
      })));
  ...
  const navItems = [
    ...
    { id: 'bank', icon: <RiBankCard2Line className="ml-2 w-5 h-5" />, text: 'حساب بانکی', link: '/bank' },
  ...
  const renderNotificationCard = (notification) => (
    <motion.div
      ...
      className={`bg-white rounded-2xl p-4 mb-4 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 ${!notification.read ? 'border-r-4 border-r-blue-500' : ''
        }`}
    >
      <div className="flex items-start gap-3">
        <button
          onClick={() => toggleSelectNotification(notification._id)}
          className={`mt-1 p-1 rounded-full transition-colors ${selectedNotifications.includes(notification._id)
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between">
            <div className="flex items-center gap-2 mb-2">
              <div className={`p-2 rounded-full ${notification.type === 'success' ? 'bg-green-100' :
              ...
              <div className="mb-6">
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl transition-colors duration-300 ${filter === 'all'
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl transition-colors duration-300 ${filter === 'unread'
                        ? 'bg-blue-600 text-white'
                        : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl transition-colors duration-300 ${filter === 'read'
                        ? 'bg-blue-600 text-white'
                        : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                  >
