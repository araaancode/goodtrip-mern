...
const TopSideButtons = () => {
  return (
    <div className="inline-block">
      <h1 className="text-xl font-bold">تیکت های پشتیبانی رانندگان</h1>
    </div>
  );
};

const DriversTickets = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle({ title: "تیکت های رانندگان" }));
    fetchTickets();
  }, []);
  ...
  const fetchTickets = async () => {
    try {
      ...
      const response = await axios.get('/api/admins/drivers/support-tickets', {
        withCredentials: true,
        timeout: 10000
      });
      
      if (response.data && response.data.data) {
        setDriverTickets(response.data.data);
      } else {
      ...
      let errorMessage = 'خطا در دریافت اطلاعات تیکت‌های رانندگان';
  ...
  const closeTicket = async (ticketId, driverId) => {
    try {
      ...
      await axios.put(`/api/admins/drivers/${driverId}/support-tickets/${ticketId}/close-ticket`, {}, {
        withCredentials: true,
        timeout: 10000
      });
      ...
      console.error('Error closing driver ticket:', err);
  ...
  const handleTicketClick = (ticket) => {
    if (ticket.status !== "Closed") {
      navigate(`/admins/drivers/${ticket.assignedTo}/support-tickets/${ticket._id}`);
    }
  };
    ...
    return (
      <TitleCard title="" topMargin="mt-2" TopSideButtons={<TopSideButtons />}>
        <div className="flex justify-center items-center h-64">
          ...
          <span className="mr-2">در حال دریافت تیکت‌های رانندگان...</span>
        </div>
      </TitleCard>
    );
  }
      ...
      {driverTickets.length > 0 ? (
                  ...
                  <td>
                    {ticket.status === "Closed" ? (
                      ...
                      <button 
                        ...
                        onClick={() => closeTicket(ticket._id, ticket.driver)}
                        title="بستن تیکت"
                      >
                        <RiCloseCircleLine />
                      </button>
        ...
        <div className="text-center py-8">
          ...
          <h3 className="text-lg font-medium text-gray-500 mt-4">
            راننده ها هنوز تیکت پشتیبانی ایجاد نکرده اند...
          </h3>
        </div>
      )}
