...
const closeTicket = (ticketId, driverId) => {
  let token = localStorage.getItem("userToken")

  // @route = /api/admins/drivers/:driverId/support-tickets/:stId/close-ticket
  axios.put(`/api/admins/drivers/${driverId}/support-tickets/${ticketId}/close-ticket`, {}, {
    headers: {
...
const DriversTickets = () => {
  ...
  useEffect(() => {
    ...
    axios.get('/api/admins/drivers/support-tickets', { headers: { authorization: AuthStr } })
      .then(response => {
    ...
    <>
      ...
      <TitleCard title="" topMargin="mt-2" TopSideButtons={<TopSideButtons />}>
        ...
        {driverTickets.length > 0 ? (
                ...
                driverTickets.map((l, k) => {
                  return (
                    <tr key={k}>
                      ...
                      {/* /admins/drivers/:userId/support-tickets */}
                      <td>
                        {l.status === "Closed" ? (<p>پاسخ داده شده</p>) : (<a href={`/admins/drivers/${l.assignedTo}/support-tickets/${l._id}`}><EditIcon /></a>)}

                      </td>
                      <td>
                        {l.status === "Closed" ? (<button className="cursor-not-allowed" disabled={true}><RiCheckboxCircleLine /></button>) : (<button onClick={() => closeTicket(l._id, l.driver)}><RiCloseCircleLine /></button>)}
                      </td>
                    </tr>
                  )
                })
          ...
          <h3>راننده ها هنوز تیکت پشتیبانی ایجاد نکرده اند...</h3>
        )}
