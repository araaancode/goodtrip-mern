...
function SingleSupportTicket() {
  const { isOwnerAuthenticated, cook: authOwner } = useOwnerAuthStore();
  ...
  const sendSupportTicket = async (e) => {
    ...
    try {
      const response = await axios.put(
        `/api/owners/support-tickets/${stId}/add-comment`,
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="font-semibold text-gray-800">
                            {c.cook ? (authOwner?.name || "کاربر") : (c.admin || "پشتیبانی")}
                          </h3>
                        </div>
              ...
              <div className="mt-8">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors duration-200">
                    ...
                    <span className="text-gray-700">{authOwner?.name || "کاربر"}</span>
                  </div>
