...
function CreateSupportTicket() {
    ...
    // Support ticket photos variables
    const [selectedFiles2, setSelectedFiles2] = useState([]);
    const fileInputRef2 = useRef(null);
    const acceptedFileExtensions2 = ["jpg", "png", "jpeg"];
    const acceptedFileTypesString2 = acceptedFileExtensions2.map((ext) => `.${ext}`).join(",");
    ...
    const addSupportTicketHandler = async (e) => {
        ...
        if (isValid) {
            try {
                ...
                const response = await axios.post(`/api/owners/support-tickets`, formData, {
                    headers: {
